...
class MY_DBSCAN:
    ...
    def __startCluster(self):
        ...
        for i in range(len(self.__cluster_border_noise)):
            if self.__cluster_border_noise[i] == 1 and self.__seen[i] == 0:
                self.label[i] = label_counter
                self.__seen[i] = 1
                # calling the recursive method
                self.__makeClusters(i, label_counter)
                label_counter +=1
